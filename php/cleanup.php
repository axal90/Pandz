 ...
 */
function pandz_root_relative_url($input) {
 ...
 */
function pandz_fix_duplicate_subfolder_urls($input) {
  $output = pandz_root_relative_url($input);
...
if (pandz_enable_root_relative_urls()) {
  ...
  add_filters($root_rel_filters, 'pandz_root_relative_url');

  add_filter('script_loader_src', 'pandz_fix_duplicate_subfolder_urls');
  add_filter('style_loader_src', 'pandz_fix_duplicate_subfolder_urls');
}
...
function pandz_gallery($attr) {
	...
	foreach ($attachments as $id => $attachment) {
		...
		if(isset($attr['slideshow']) && 'true' == $attr['slideshow']) {
			...
			if(isset($attr['link']) && 'file' == $attr['link'] && 'true') {
				$link = str_replace('<a class="', '<a rel="'.$selector.'" class="thickbox ',  $link);
			}
		}
		$output .= "
	...
	$output .= "</ul>\n";
	if(isset($attr['slideshow']) && 'true' == $attr['slideshow'])
		$output .= "<div id=\"{$pager_id}\" class=\"pagers\"></div></div>\n";
 ...
 */
function pandz_get_search_form() {
  locate_template('/templates/searchform.php', true, true);
}

add_filter('get_search_form', 'pandz_get_search_form');
